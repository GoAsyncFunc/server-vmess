...
func (d *DefaultDispatcher) Dispatch(ctx context.Context, destination net.Destination) (*transport.Link, error) {
	...
	go d.routedDispatch(ctx, outbound)
	return inbound, nil
}
...
func (d *DefaultDispatcher) routedDispatch(ctx context.Context, link *transport.Link) {
	outbounds := session.OutboundsFromContext(ctx)
	ob := outbounds[len(outbounds)-1]
	...
	// Remove forced outbound logic, use default handler directly
